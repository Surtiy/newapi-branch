...
func TokenAuth() func(c *gin.Context) {
	return func(c *gin.Context) {
		...
		if len(tokenGroups) > 0 {
			allowed, denied, groupErr := resolveUsableTokenGroups(userGroup, tokenGroups)
			if groupErr != nil && len(allowed) == 0 {
				abortWithOpenAiMessage(c, http.StatusForbidden, groupErr.Error())
				return
			}
			c.Set(deniedTokenGroupsKey, denied)
			token.Groups = model.NewTokenGroups(allowed)
			token.Group = allowed[0]
			userGroup = allowed[0]
		}
...
func SetupContextForToken(c *gin.Context, token *model.Token, parts ...string) error {
	...
	tokenGroups := token.GetGroups()
	if len(tokenGroups) > 0 {
		common.SetContextKey(c, constant.ContextKeyTokenGroup, tokenGroups[0])
		common.SetContextKey(c, constant.ContextKeyTokenGroups, tokenGroups)
	} else {
		common.SetContextKey(c, constant.ContextKeyTokenGroup, token.Group)
		common.SetContextKey(c, constant.ContextKeyTokenGroups, []string{})
	}
