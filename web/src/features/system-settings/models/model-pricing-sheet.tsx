...
type ModelPricingSheetProps = {
  ...
  /** Video models expose the branch-friendly request/second pricing choices. */
...
>(function ModelPricingEditorPanel(
  ...
  const supportsPerSecond =
    isVideoModel ||
    editData?.billingMode === 'per-second' ||
    taskUsageSchema?.seconds?.unit === 'second'
  ...
  useEffect(() => {
    ...
    if (editData?.billingMode === 'per-second') originalMode = 'per-second'
    else if (editData?.billingMode === 'tiered_expr') originalMode = 'tiered_expr'
  ...
  const warnings = useMemo(() => {
    ...
    if (pricingMode === 'per-second' && !hasValue(watchedValues.price)) {
      nextWarnings.push(t('Per-second price is required before saving.'))
    }
  ...
  const validatePricingValues = useCallback(() => {
    ...
    if (pricingMode === 'per-second' && !hasValue(form.getValues('price'))) {
      form.setError('price', {
        message: t('Per-second price is required before saving.'),
      })
      return false
    }
  ...
  const buildSubmitData = useCallback(
    (values: ModelPricingFormValues) => {
      const data: ModelRatioData = {
        ...
        ...(isVideoModel ? { isVideoModel: true } : {}),
      }
    ...
    [isVideoModel, pricingMode, requestRuleExpr, resolvedBillingExpr]
  )
            ...
            <div className='grid min-w-0 items-start gap-4 @min-[960px]/pricing-editor:grid-cols-[minmax(0,1fr)_260px]'>
              <FieldGroup className='min-w-0'>
                ...
                >
                  ...
                  >
                    {!isVideoModel && <TabsTrigger value='per-token'>
                      {t('Per-token')}
                    </TabsTrigger>}
                    ...
                    {!isVideoModel && <TabsTrigger value='tiered_expr'>
                      {t('Expression')}
                    </TabsTrigger>}
                  </TabsList>
                          ...
                          render={({ field }) => (
                            <FormItem className='contents'>
                              <Field>
                                ...
                                <FormDescription>
                                  {t(
                                    'Cost in {{currency}} for each second of generated media.',
                                    { currency: currency.label }
                                  )}
                                </FormDescription>
                                <FormMessage />
                  ...
                  {!isVideoModel && <TabsContent value='tiered_expr' className='pt-0'>
                    <FieldGroup className='gap-5'>
                      {taskUsageSchema ? (
