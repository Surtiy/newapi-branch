...
function isVideoPricingModel(model: {
  model_name?: string
  ...
  if (model.output_modalities?.some((value) => value.toLowerCase() === 'video')) return true
  // Older catalog payloads omitted endpoint metadata; retain compatibility for
  // explicitly video-labelled models while avoiding broad substring matches.
  return /(^|[-_:])(video|sora)([-_:]|$)/i.test(model.model_name || '') ||
    /(^|[,\s])video([,\s]|$)/i.test(model.tags || '')
}

const looksLikeVideoModel = (name: string) =>
  /(^|[-_:])(video|sora|seedance|kling|hailuo|runway)([-_:]|$)/i.test(name)
...
>(function ModelRatioVisualEditor(
  ...
  const modeCounts = useMemo(() => {
    ...
    for (const model of models) {
      let mode: keyof typeof counts = 'per-token'
      if (
  ...
  const handleEdit = useCallback(
    (model: ModelRow) => {
      ...
      if (editableModel.billingMode === 'per-second') {
          ...
          {editorOpen ? (
            <ModelPricingEditorPanel
              ...
              isVideoModel={Boolean(
                editData?.name &&
                (videoModelNames.has(editData.name) ||
                  looksLikeVideoModel(editData.name))
              )}
              onSave={onSave}
              isSaving={isSaving}
