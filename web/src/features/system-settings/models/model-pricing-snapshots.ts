...
export type SimpleUnitPricing = {
  unit: 'request' | 'second'
  price: string
}

export function parseSimpleUnitPricingExpression(
  expression?: string
): SimpleUnitPricing | null {
  if (!expression) return null

  const normalized = expression.trim()
  const requestMatch = normalized.match(
    /^tier\(\s*"request"\s*,\s*([+\-]?\d+(?:\.\d+)?(?:[eE][+\-]?\d+)?)\s*\)$/
  )
  if (requestMatch) {
    const price = Number(requestMatch[1])
    return Number.isFinite(price) && price >= 0
      ? { unit: 'request', price: formatPricingNumber(price) }
      : null
  }

  const secondMatch = normalized.match(
    /^tier\(\s*"second"\s*,\s*u\(\s*"seconds"\s*\)\s*\*\s*([+\-]?\d+(?:\.\d+)?(?:[eE][+\-]?\d+)?)\s*\)$/
  )
  if (secondMatch) {
    const price = Number(secondMatch[1])
    return Number.isFinite(price) && price >= 0
      ? { unit: 'second', price: formatPricingNumber(price) }
      : null
  }
...
export const buildModelSnapshots = ({
  ...
  return [...modelNames].map((name) => {
    ...
    if (modeForModel === 'tiered_expr') {
      ...
      const simplePricing = parseSimpleUnitPricingExpression(pureExpr)
      let displayBillingMode: ModelPricingSnapshot['billingMode'] = 'tiered_expr'
      if (simplePricing?.unit === 'second') {
        displayBillingMode = 'per-second'
      } else if (simplePricing?.unit === 'request') {
        displayBillingMode = 'per-request'
      }
      return {
        name,
        billingMode: displayBillingMode,
        billingExpr: pureExpr,
        requestRuleExpr,
        price: simplePricing?.price || price,
        ratio,
        cacheRatio: cache,
