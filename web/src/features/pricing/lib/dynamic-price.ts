...
export function getDynamicPricingSummary(
  ...
  // Video pricing uses a compact canonical expression but may not include a
  // usage schema in older catalog responses. Recognize it directly so cards
  // show a friendly per-request/per-second price instead of raw internals.
  const simpleVideo = model.billing_expr?.trim().match(
    /^tier\("(request|second)",\s*(?:u\("seconds"\)\s*\*\s*)?([\d.eE+-]+)\)$/
  )
  if (simpleVideo) {
    const unit = simpleVideo[1] === 'second' ? 'second' : 'request'
    const value = Number(simpleVideo[2])
    if (Number.isFinite(value) && value >= 0) {
      const entry: DynamicPriceEntry = {
        key: unit,
        field: unit,
        label: unit,
        shortLabel: unit,
