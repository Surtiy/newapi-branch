...
export function getApiKeyFormSchema(t: TFunction, maxAutoGroups = 5) {
  ...
  return z
    .object({
      ...
      groups: z.array(z.string()).min(1, t('Please select a group')),
...
export function transformFormDataToPayload(
  ...
  const groups = Array.from(new Set((data.groups?.length ? data.groups : [data.group || '']).filter(Boolean)))
  ...
  return {
    ...
    groups,
    auto_groups:
      group === 'auto' && data.auto_groups_mode === 'custom'
        ? data.auto_groups
        : [],
...
export function transformApiKeyToFormDefaults(
  ...
  const groups = apiKey.groups?.length ? apiKey.groups : [apiKey.group || DEFAULT_GROUP]
