                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <MultiSelect
                        ...
                        onChange={(values) => {
                          const nextValues = values.includes('auto')
                            ? ['auto']
                            : values
                          field.onChange(nextValues)
                          form.setValue('group', nextValues[0] || '')
                          form.setValue('cross_group_retry', nextValues[0] === 'auto', { shouldDirty: true })
                        }}
                        placeholder={t('Select one or more groups')}
                      />
                    </FormControl>
                    <FormDescription>{t('The first selected group is used by default; failed requests will try the following groups in order.')}</FormDescription>
                    <FormMessage />
                  </FormItem>
                )}
              />
