...
export function pricingRow(
  ...
  if (row?.billingMode === 'per-second') billingMode = 'per-second'
  else if (row?.billingMode === 'tiered_expr') billingMode = 'tiered_expr'
...
export function pricingFromDraft(data: ModelRatioData): PricingValues {
  const values: PricingValues = {
    'billing_setting.billing_mode':
      data.billingMode === 'tiered_expr' || data.billingMode === 'per-second'
        ? 'tiered_expr'
        : 'ratio',
  }
  for (const [field, key] of Object.entries(pricingFieldMap)) {
    ...
    let shouldPersist = data.billingMode === 'tiered_expr'
    if (data.billingMode === 'per-request') {
      shouldPersist = key === 'ModelPrice'
    } else if (data.billingMode === 'per-token') {
      shouldPersist = key !== 'ModelPrice'
    }
    if (shouldPersist) {
      values[key] = number
  ...
  if (data.billingMode === 'per-second') {
    ...
    if (price !== null && Number.isFinite(price) && price >= 0) {
      values['billing_setting.billing_expr'] = `tier("second", u("seconds") * ${price})`
    }
  }
  if (data.isVideoModel && data.billingMode === 'per-request') {
    const price = data.price === undefined || data.price === '' ? null : Number(data.price)
    if (price !== null && Number.isFinite(price) && price >= 0) {
      delete values.ModelPrice
      values['billing_setting.billing_mode'] = 'tiered_expr'
      values['billing_setting.billing_expr'] = `tier("request", ${price})`
    }
  }
  return values
}
...
function applyPricingValues(
  ...
  const maps = Object.fromEntries(
    PRICING_KEYS.map((key) => [
      key,
      JSON.parse(options[key]) as Record<string, number | string>,
    ])
  ) as Record<PricingKey, Record<string, number | string>>

  // A model has one active pricing mode. Remove every old representation
  // before writing the new one so switching request/second/expression modes
  // cannot leave stale ModelPrice, ratios, or billing expressions behind.
  for (const name of names) {
    for (const key of PRICING_KEYS) delete maps[key][name]
    for (const [key, value] of Object.entries(values) as Array<
      [PricingKey, number | string]
    >) {
      maps[key][name] = value
    }
  }
...
export function applyPriceSyncSelections(
  ...
  for (const [name, fields] of Object.entries(selections)) {
    ...
    if (expression) {
      ...
      validated['billing_setting.billing_mode'] = 'tiered_expr'
