...
export function ModelPricingPanel(props: {
  ...
  const isVideoModel = pricingModels.some((model) =>
    model.model_name === entry.model_name && (
      model.supported_endpoint_types?.includes('openai-video') ||
      model.output_modalities?.some((value) => value.toLowerCase() === 'video') ||
      /(^|[-_:])(video|sora|seedance|kling|hailuo|runway)([-_:]|$)/i.test(model.model_name) ||
      /(^|[,\s])video([,\s]|$)/i.test(model.tags || '')
    )
  ) || entry.usage_schema?.seconds?.unit === 'second'
