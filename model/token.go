...
func NewTokenGroups(groups []string) TokenGroups {
	...
	return TokenGroups(data)
}

func (groups TokenGroups) List() []string {
	raw := strings.TrimSpace(string(groups))
	if raw == "" {
		return []string{}
	}
	var values []string
	if json.Unmarshal([]byte(raw), &values) == nil {
		return normalizeTokenGroups(values)
	}
	return normalizeTokenGroups(strings.Split(raw, ","))
}

func (groups TokenGroups) MarshalJSON() ([]byte, error) {
	return json.Marshal(groups.List())
}

func (groups *TokenGroups) UnmarshalJSON(data []byte) error {
	if groups == nil {
		return nil
	}
	var values []string
	...
	*groups = NewTokenGroups(strings.Split(value, ","))
	return nil
}

func (groups TokenGroups) Value() (driver.Value, error) {
	return string(groups), nil
}

func (groups *TokenGroups) Scan(value interface{}) error {
	switch v := value.(type) {
	case nil:
		*groups = ""
	case string:
		*groups = TokenGroups(v)
	case []byte:
		*groups = TokenGroups(string(v))
	default:
		return fmt.Errorf("unsupported TokenGroups type: %T", value)
	}
	return nil
}
...
type Token struct {
	...
	Groups             TokenGroups    `json:"groups" gorm:"type:text;default:''"`
	CrossGroupRetry    bool           `json:"cross_group_retry"` // 跨分组重试，仅auto分组有效
	AutoGroups         string         `json:"-" gorm:"type:text"`
	DeletedAt          gorm.DeletedAt `gorm:"index"`
}

func (token *Token) NormalizeGroups() {
	if token == nil {
		return
	}
	groups := token.Groups.List()
	if len(groups) == 0 && strings.TrimSpace(token.Group) != "" {
		groups = []string{token.Group}
	}
	token.Groups = NewTokenGroups(groups)
	if len(groups) > 0 {
...
func (token *Token) Update() (err error) {
	...
	return DB.Model(token).Select("name", "status", "expired_time", "remain_quota", "unlimited_quota",
		"model_limits_enabled", "model_limits", "allow_ips", "group", "groups", "cross_group_retry", "auto_groups").Updates(token).Error
}
