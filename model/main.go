...
func migrateDB() error {
	...
	if err := migrateTokenGroupsFromGroup(); err != nil {
		return err
	}
	return nil
}

func migrateTokenGroupsFromGroup() error {
	if !DB.Migrator().HasTable("tokens") || !DB.Migrator().HasColumn(&Token{}, "groups") {
		return nil
	}
	var tokens []Token
	if err := DB.Select("id", commonGroupCol, "groups").Where("(groups IS NULL OR groups = '') AND "+commonGroupCol+" <> ''").Find(&tokens).Error; err != nil {
		return err
	}
	for _, token := range tokens {
		groups := NewTokenGroups([]string{token.Group})
		if groups != "" {
			if err := DB.Model(&Token{}).Where("id = ?", token.Id).Update("groups", groups).Error; err != nil { return err }
		}
	}
	return nil
}
