...
func getTokenFallbackGroups(ctx *gin.Context, fallbackGroup string) []string {
	if ctx != nil {
		if raw, exists := common.GetContextKey(ctx, constant.ContextKeyTokenGroups); exists {
			if groups, ok := raw.([]string); ok && len(groups) > 1 { return groups }
		}
		// Task-plugin routes can rebuild the request context without the derived
		// multi-group value. Recover it from the authenticated token instead of
		// silently restricting this request to its first group.
		if tokenID := ctx.GetInt("token_id"); tokenID > 0 {
			if token, err := model.GetTokenById(tokenID); err == nil && token != nil {
				token.NormalizeGroups()
				if groups := token.GetGroups(); len(groups) > 1 { return groups }
			}
		}
		if tokenKey := ctx.GetString("token_key"); tokenKey != "" {
			if token, err := model.ValidateUserToken(tokenKey); err == nil && token != nil {
				token.NormalizeGroups()
				if groups := token.GetGroups(); len(groups) > 0 { return groups }
			}
		}
	}
	if fallbackGroup == "" { return []string{} }
	return []string{fallbackGroup}
}

func setSelectedTokenGroup(ctx *gin.Context, group string) {
	if ctx == nil || group == "" { return }
...
func CacheGetRandomSatisfiedChannel(param *RetryParam) (*model.Channel, string, error) {
	...
	if param.Ctx != nil {
		if raw, ok := common.GetContextKey(param.Ctx, constant.ContextKeyTokenGroups); ok {
			logger.LogDebug(param.Ctx, "token group selection token_group=%s token_groups_type=%T token_groups=%v model=%s", param.TokenGroup, raw, raw, param.ModelName)
		} else {
			logger.LogDebug(param.Ctx, "token group selection token_group=%s token_groups_missing model=%s", param.TokenGroup, param.ModelName)
		}
	}
	...
	if param.TokenGroup == "auto" {
	...
	} else if tokenGroups := getTokenFallbackGroups(param.Ctx, param.TokenGroup); len(tokenGroups) > 1 {
		start := 0
		if raw, exists := common.GetContextKey(param.Ctx, constant.ContextKeyTokenGroupIndex); exists {
			if idx, ok := raw.(int); ok && idx >= 0 { start = idx }
		}
		for i := start; i < len(tokenGroups); i++ {
			group := tokenGroups[i]
			priorityRetry := param.GetRetry()
			if i > start { priorityRetry = 0 }
			channel, err = model.GetRandomSatisfiedChannel(group, param.ModelName, priorityRetry, filters)
			if err != nil { return nil, group, err }
			if channel == nil {
			...
			selectGroup = group
			setSelectedTokenGroup(param.Ctx, group)
			if priorityRetry >= common.RetryTimes && i+1 < len(tokenGroups) {
				common.SetContextKey(param.Ctx, constant.ContextKeyTokenGroupIndex, i+1)
				param.SetRetry(0)
				param.ResetRetryNextTry()
			} else { common.SetContextKey(param.Ctx, constant.ContextKeyTokenGroupIndex, i) }
			break
		}
	} else {
		...
		setSelectedTokenGroup(param.Ctx, param.TokenGroup)
	}
